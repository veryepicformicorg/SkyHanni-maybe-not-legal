...
object PestHighlighter {
    ...
    const val DRAGONFLY_PEST: String =
    ...
    const val EARTHWORM_PEST: String =
    ...
    const val EARTHWORM_PEST_TAIL: String =
    ...
    const val FIELD_MOUSE_PEST: String =
    ...
    const val FIREFLY_PEST_FLASH: String =
    ...
    const val LUNAR_MOTH_PEST: String =
    ...
    const val MOSQUITO_PEST: String =
    ...
    const val PRAYING_MANTIS_PEST: String =
    ...
    @HandleEvent(SkyHanniRenderWorldEvent::class, onlyOnIsland = IslandType.GARDEN)
    fun onRenderWorld(event: SkyHanniRenderWorldEvent) {
        if (!config.enabled.get() || !config.showTracers) return


        for (pest in pests) {
            val location = event.exactLocation(pest)
            event.drawLineToCrosshair(location.add(0.0, 1.5), java.awt.Color(255, 0, 0), 3, true)
        }
